...
class MetaProcesFormat():
    ...
    def return_date_list(first_date: str, meta_key: str, s3_bucket_meta: S3BucketConnector):
        """
        Creating a lists of date based on the input first_date and the already processed date in the meta file

        :param: first_date - the earliest date that should be extracted
        :param: meta_key - key of the meta file in the S3 bucket
        :param: s3_bucket_meta - S3BucketConnector to connect to the bucket

        returns:
            min_date: first date that should be processed
            return_date_list  list of all date ranging from min_date to current date
        """
